export class Sort {
	...
	public static RecordArrayByKey = <T>(arg: Record<string, T[]>): Record<string, T[]> => {
		...
		let packages: string[] = Object.keys(arg).filter(f => f.startsWith('../'))
		let others: string[] = Object.keys(arg).filter(f => !internal.includes(f) && !packages.includes(f))
		
		internal.sort((a: string, b: string) => a > b ? 1 : -1)
		packages.sort((a: string, b: string) => a > b ? 1 : -1)
		others.sort((a: string, b: string) => a > b ? 1 : -1)
		
		const result: Record<string, T[]> = {}
		others.forEach((m: string) => result[m] = arg[m].sort((a, b) => a > b ? 1 : -1))
		packages.forEach((m: string) => result[m] = arg[m].sort((a, b) => a > b ? 1 : -1))
